...
export const findMarqueee = async () => {
    ...
    try {
        const { data } = await axios.get('http://localhost:7913/api/v1/admin/annoncements');
...
export const findNavs = async () => {
    try {
        const { data } = await axios.get('http://localhost:7913/api/v1/admin/category');
...
export const findMyLastOrder = async () => {
    try {
        const { data } = await axios.get('http://localhost:7913/api/v1/my-last-order', {
            headers: {
                Authorization: `Bearer ${sessionStorage.getItem('token_login')}`,
            },
        });
...
export const findMyAllOrders = async () => {
    try {
        const { data } = await axios.get('http://localhost:7913/api/v1/my-all-order', {
            headers: {
                Authorization: `Bearer ${sessionStorage.getItem('token_login')}`,
            },
        });
...
export const ApiHandleLogout = async () => {
    try {
        const { data } = await axios.get('http://localhost:7913/api/v1/logout', {
            headers: {
                Authorization: `Bearer ${sessionStorage.getItem('token_login')}`,
            },
        });
