...
const CheckoutFlow = () => {
  ...
  useEffect(() => {
    const boot = async () => {
      try {
        // Fetch settings
        const settingsRes = await axios.get("http://localhost:7912/api/v1/admin/settings");
        ...
        if (token) {
          ...
          try {
            const lastOrder = await axios.get("http://localhost:7912/api/v1/my-last-order", {
              headers: { Authorization: `Bearer ${token}` },
            });
  ...
  const handleGuestSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:7912/api/v1/create_user_from_cart", { Email, ContactNumber });
      if (res.data?.success) {
    ...
    if (paymentMethod === "COD") {
      try {
        const res = await axios.post(
          "http://localhost:7912/api/v1/create-cod-order",
        ...
        handler: async (response) => {
          ...
          try {
            const verify = await axios.post(
              "http://localhost:7912/api/v1/verify-payment",
              response
            );
