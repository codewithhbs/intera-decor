...
const SubCategory = ({ isOpen = true, onClose, selectedCategory }) => {
    ...
    const fetchCategories = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:7912/api/v1/admin/sub-category/${selectedCategory}`);
    ...
    const updateSubCategory = async (subCategoryId) => {
        setLoading(true);
        try {
            await axios.put(`http://localhost:7912/api/v1/admin/sub-category/edit/${subCategoryId}`, { name });
    ...
    const deleteSubCategory = async (subCategoryId) => {
        if (window.confirm('Are you sure you want to delete this subcategory?')) {
            setLoading(true);
            try {
                await axios.delete(`http://localhost:7912/api/v1/admin/sub-category/delete/${subCategoryId}`);
