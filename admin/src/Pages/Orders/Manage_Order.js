...
const ManageOrder = () => {
  ...
  const fetchOrders = async (page) => {
    try {
      setLoading(true);
      const response = await fetch(`https://api.interdecor.adsdigitalmedia.comapi/v1/admin/get-all-order?page=${page}&limit=6`);
  ...
  const handleStatusChange = async (orderId, status) => {
    try {
      ...
      setLoading(true);
      const response = await fetch("https://api.interdecor.adsdigitalmedia.comapi/v1/admin/change-order-status", {
        method: 'POST',
        headers: {
  ...
  const handleDeleteOrder = async (orderId) => {
    if (window.confirm("Are you sure you want to delete this order?")) {
      try {
        setLoading(true);
        await fetch(`https://api.interdecor.adsdigitalmedia.comapi/v1/admin/delete-order/${orderId}`, {
          method: 'DELETE',
        });
        fetchOrders(currentPage);
