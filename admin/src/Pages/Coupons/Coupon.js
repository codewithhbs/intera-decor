...
const Coupon = () => {
  ...
  const fetchCoupons = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://api.interdecor.adsdigitalmedia.com/api/v1/get-coupon');
  ...
  const createAndEditCoupon = async (type) => {
    try {
      setLoading(true);
      if (type === 'edit') {
        await axios.post(`https://api.interdecor.adsdigitalmedia.com/api/v1/update-coupon/${selectedCoupon?.code}`, formData);
      } else {
        await axios.post('https://api.interdecor.adsdigitalmedia.com/api/v1/add-coupon', formData);
      }
  ...
  const deleteCoupon = async (code) => {
    try {
      setLoading(true);
      await axios.delete(`https://api.interdecor.adsdigitalmedia.com/api/v1/delete-coupon/${code}`);
