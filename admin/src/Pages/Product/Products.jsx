...
const Products = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:7912/api/v1/get-product')
      .then(response => {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      const res = await axios.delete(`http://localhost:7912/api/v1/delete-product/${id}`)
      if (res.data.success) {
  ...
  const handleToggleShowOnHome = async (productId, currentValue) => {
    try {
      const response = await axios.put(`http://localhost:7912/api/v1/update-show-home/${productId}`, {
